...
router.post('/', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const msg = formatErrorMessage(err, 'Failed to create super admin')
...
router.post('/verify-otp', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const msg = formatErrorMessage(err, 'Verification failed')
    res.status(400).json({ error: msg })
  }
})
...
router.patch('/:id', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const msg = formatErrorMessage(err, 'Failed to update super admin')
    res.status(400).json({ error: msg })
  }
})
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const msg = formatErrorMessage(err, 'Failed to delete super admin')
    res.status(400).json({ error: msg })
  }
})
